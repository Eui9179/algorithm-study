...
class Solution {
    ...
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        List<int[]> waitingQueue = genQueue(truck_weights, bridge_length); // [truck weight, time]
        List<int[]> passing = new ArrayList<>();

        while (!waitingQueue.isEmpty() || !passing.isEmpty()) {
            reduceTimeOrPop(passing);
            answer++;

            if (!waitingQueue.isEmpty() && passing.isEmpty()) {
                passing.add(waitingQueue.remove(0));
                continue;
            }

            if (!waitingQueue.isEmpty()) {
                int[] truck = waitingQueue.get(0);
                int truckWeight = truck[0];
                if (passing.size() < bridge_length && sum(passing, truckWeight) <= weight) {
                    passing.add(waitingQueue.remove(0));
                }
            }
    ...
    private void reduceTimeOrPop(List<int[]> passing) {
        if (passing.isEmpty()) return;

        for (int[] pass : passing) {
            pass[1]--;
        }

        while (!passing.isEmpty() && passing.get(0)[1] == 0) {
            passing.remove(0);
        }
    }

    private List<int[]> genQueue(int[] trucks, int time) {
        List<int[]> list = new ArrayList<>();
        for (int truck : trucks) {
            list.add(new int[]{truck, time});
        }
        return list;
    }

    private int sum(List<int[]> passing, int truck) {
        if (passing.isEmpty()) return 0;

        int result = 0;
        for (int[] pass : passing) result += pass[0];
        return result + truck;
    }
}