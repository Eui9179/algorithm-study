package org.codingtest.score_kit.stackandqueue.p42583;
...
class Solution {
    ...
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        for (int truck : truck_weights) {
            wait.offer(new Truck(bridge_length, truck));
        }

        while (!bridge.isEmpty() || !wait.isEmpty()) {
            bridge.forEach(b -> b.timer -= 1);

            if (!bridge.isEmpty() && bridge.peek().timer == 0) bridge.poll();

            int sum = sumBridge(bridge);
            if (!wait.isEmpty() && sum + wait.peek().weight <= weight) {
    ...
    private static class Truck {
        ...
        private Truck(int timer, int weight) {
            this.timer = timer;
            this.weight = weight;
        }
    }

    private int sumBridge(Queue<Truck> bridge) {
