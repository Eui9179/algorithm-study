package org.codingtest.score_kit.hash.p42576;
...
class Solution {
    ...
    public String solution(String[] participant, String[] completion) {
        ...
        Map<String, Integer> compMap = Arrays.stream(completion).collect(
                Collectors.toMap(
                        comp -> comp,
                        comp -> 1,
        ...
        for (String part : participant) {
            if (!compMap.containsKey(part) || compMap.get(part) <= 0) {
                answer = part; break;
            }
            compMap.put(part, compMap.get(part) - 1);
        }
