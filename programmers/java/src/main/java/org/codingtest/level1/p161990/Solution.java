package org.codingtest.level1.p161990;
...
이러한 바탕화면에서 다음 그림과 같이 S(0, 1)에서 E(3, 4)로 드래그하면 세 개의 파일이 모두
선택되므로 드래그 한 거리 (3 - 0) + (4 - 1) = 6을 최솟값으로 모든 파일을 선택 가능합니다.

eg1-2.png
(0, 0)에서 (3, 5)로 드래그해도 모든 파일을 선택할 수 있지만 이때 드래그 한 거리는 (3 - 0) + (5 - 0) = 8이고 이전의 방법보다 거리가 늘어납니다.

머쓱이의 컴퓨터 바탕화면의 상태를 나타내는 문자열 배열 wallpaper가 매개변수로 주어질 때 바탕화면의 파일들을 한 번에 삭제하기 위해 최소한의
이동거리를 갖는 드래그의 시작점과 끝점을 담은 정수 배열을 return하는 solution 함수를 작성해 주세요.
...
class Solution {
    ...
    private int[] getPoint(String[] wallpaper) {
        int top = wallpaper.length, left = wallpaper[0].length(), bottom = 0, right = 0;

        String[][] wallpapers = getWallpapers(wallpaper);

        for (int i = 0; i < wallpapers.length; i++) {
            for (int j = 0; j < wallpapers[0].length; j++) {
    ...
    private String[][] getWallpapers(String[] wallpaper) {
        String[][] wallpapers = new String[wallpaper.length][wallpaper[0].length()];
        for (int i = 0; i < wallpaper.length; i++) wallpapers[i] = wallpaper[i].split("");
        return wallpapers;
    }
}