package org.codingtest.level1.p138477;
...
class Solution {
    public static void main(String[] args) {
        System.out.println(Arrays.toString(new Solution().solution(3, new int[]{10, 100, 20, 150, 1, 100, 200})));
        System.out.println(Arrays.toString(new Solution().solution(4, new int[]{0, 300, 40, 300, 20, 70, 150, 50, 500, 1000})));
    }
    ...
    public int[] solution(int k, int[] score) {
        ...
        for (int i = 0; i < score.length; i++) {
            if (record.size() < k) {
                record.add(score[i]);
            } else if (record.peek() != null && record.peek() < score[i]) {
                record.poll();
                record.add(score[i]);
            }
            if (record.peek() != null) answer[i] = record.peek();
        }
        return answer;
    }
}