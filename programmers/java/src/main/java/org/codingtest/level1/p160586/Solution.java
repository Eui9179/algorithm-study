package org.codingtest.level1.p160586;
/*
...
class Solution {
    public int[] solution(String[] keymap, String[] targets) {
        ...
        Map<Character, Integer> alphaMap = getAlphaMap(keymap);

        for (int i = 0; i < targets.length; i++) {
            for (int j = 0; j < targets[i].length(); j++) {
                Character alpha = targets[i].charAt(j);
                if (!alphaMap.containsKey(alpha)) {
                    answer[i] = -1;
                    break;
                }
                answer[i] += alphaMap.get(alpha);
            }
        }
    ...
    private Map<Character, Integer> getAlphaMap(String[] keymap) {
